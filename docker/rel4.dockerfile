...
# RUN set -eux; \
#     git clone \
#         https://github.com/reL4team2/rel4-integral.git rel4_kernel -b feature/microkit \
#         --config advice.detachedHead=false;
# RUN set -eux; \
#     git clone \
#         https://github.com/reL4team2/seL4_c_impl.git \
#         --config advice.detachedHead=false -b feature/microkit;
...
RUN cargo install --git https://github.com/reL4team2/rust-sel4.git \ 
                  --rev 642b58d807c5e5fc22f0c15d1467d6bec328faa9 \
                  --root ${REL4_PREFIX} \
                  sel4-kernel-loader-add-payload;

RUN cargo install -Z build-std=core,compiler_builtins \
                -Z build-std-features=compiler-builtins-mem \
                --target aarch64-unknown-none \
